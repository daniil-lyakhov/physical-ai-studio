...
class TimedPolicyWrapper:
    ...
    def print_stats(self, suite_name: str = "") -> None:
        ...
        # ActionChunking makes most calls fast (cached) and every Nth slow (full inference).
        # Split into full-inference vs cached using a threshold at 2x median.
        median = np.median(arr)
        full_mask = arr > 2 * median
        if full_mask.any() and (~full_mask).any():
            full = arr[full_mask]
            cached = arr[~full_mask]
            print(f"\n  --- Full inference calls (>{2 * median:.1f} ms) ---")
            print(f"  Count       : {len(full)}")
            print(f"  Mean        : {full.mean():.2f} ms")
            print(f"  Min / Max   : {full.min():.2f} / {full.max():.2f} ms")
            print(f"\n  --- Cached chunk returns ---")
            print(f"  Count       : {len(cached)}")
            print(f"  Mean        : {cached.mean():.2f} ms")
        print(f"{'=' * 60}\n")
