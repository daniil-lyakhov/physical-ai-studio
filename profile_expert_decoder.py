"""Profile gemma_expert_decoder activations using NNCF activation profiler.

Loads calibration data from safetensors (output of collect_calib_datasets.py),
profiles the FP32 expert decoder, optionally compares with INT4, and outputs
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Profile gemma_expert_decoder activations.")
    ...
    parser.add_argument("--calibration-file", type=str, required=True, help="Path to expert_decoder_calibration.safetensors")
    parser.add_argument("--output-dir", type=str, default="./profiling_results_expert", help="Output directory")
    ...
    # Load FP32 gemma_expert_decoder
    core = openvino.Core()
    fp32_path = Path(args.model_dir) / "gemma_expert_decoder.xml"
    ...
    if args.compressed_model_dir:
        int4_path = Path(args.compressed_model_dir) / "gemma_expert_decoder.xml"
